...
class NewRepackgingChalanListUi extends StatefulWidget {
  String id ;
  NewRepackgingChalanListUi({Key? key,required this.id}) : super(key: key);
  @override
  State<NewRepackgingChalanListUi> createState() => _NewRepackgingChalanListUiState();
}

class _NewRepackgingChalanListUiState extends State<NewRepackgingChalanListUi> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Chalan RP",style: TextStyle(fontSize: 16),),
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            // ElevatedButton(
            //     onPressed: ()=>OpenDialogChalanRepackage(context),
  ...
  popAndLoadPage() {
    ...
    Navigator.pop(context);
    goToPage(context, NewRepackgingChalanListUi(id:widget.id));
  }
  ...
  Future<List<Results>?> pickUpOrders() async {
    ...
    log('https://${finalUrl}${StringConst.getChalanDetailInfo}?id=&chalan_master=${widget.id}');
