...
class RePackAll extends StatefulWidget {
  String masterId;
  List packCodes =[];
  Map<String, String> serialvalueDict = {};


  RePackAll(this.masterId,this.packCodes,this.serialvalueDict);

  @override
  State<RePackAll> createState() => _RePackAllState();
}

class _RePackAllState extends State<RePackAll> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('All Repackaging'),
  ...
  Future dropCurrentItem(locationCode, packCode)  async {
    ...
    final response = await http.post(
        Uri.parse('${StringConst.protocol}$finalUrl${StringConst.postRePackaging}'),
        headers: {
          'Content-type': 'application/json',
          'Accept': 'application/json',
          'Authorization': 'Bearer ${prefs.get("access_token")}'
