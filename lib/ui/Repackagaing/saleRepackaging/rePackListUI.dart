...
import 'package:indigo_paints/ui/Repackagaing/model/listingsModel.dart';

import 'newSaleRepackage/repackageListings.dart';

class RePackListUi extends StatefulWidget {
  const RePackListUi({Key? key}) : super(key: key);

  @override
  State<RePackListUi> createState() => _RePackListUiState();
}

class _RePackListUiState extends State<RePackListUi> {
  ...
  Future<List<Results>?> RepackagingListServices(String search) async {
    ...
    final response = await http.get(
        Uri.parse('https://$finalUrl${StringConst.rePackList}?ordering=-id'),
        // Uri.parse('http://$finalUrl:8081${StringConst.rePackList}?ordering=-id'),
        headers: {
    ...
    return data != null
        ...
        itemBuilder: (context, index) {
          return GestureDetector(
            onTap: (){
              goToPage(context,
                  // RepackgingListUi(id: data[index].id.toString()));
                  NewRepackgingListUi(id: data[index].id.toString()));
            },
            child: Card(
