...
import 'package:indigo_paints/ui/department%20transfer/model/pack_type_model.dart';
import 'package:indigo_paints/ui/department%20transfer/pickup/pickupUi.dart';
import 'package:indigo_paints/ui/department%20transfer/pickup/scanPackToPick.dart';
import 'package:provider/provider.dart';
import 'package:shared_preferences/shared_preferences.dart';
import 'package:sn_progress_dialog/progress_dialog.dart';
import 'package:indigo_paints/ui/pick/ui/ByBatch/testPickUpByBatch.dart';
...
class DepartmentPickupScanLocation extends StatefulWidget {
  int orderId;
  int purchaseDetail;
  double qty;

  DepartmentPickupScanLocation(this.purchaseDetail, this.orderId, this.qty);

  @override
  State<DepartmentPickupScanLocation> createState() =>
      _DepartmentPickupScanLocationState();
}

class _DepartmentPickupScanLocationState
    extends State<DepartmentPickupScanLocation> {
  ...
  Widget build(BuildContext context) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      // widget.qty == widget.qty ? _completedButton(context) : null;
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Text('Save Department Pickup Codes',style: TextStyle(fontSize: 14),),
                                  ...
                                  child: Center(
                                      child: Padding(
                                        ...
                                        child: Column(
                                          children: [
                                            Text(
                                              "${(widget.qty.toInt() - pvr.serialId.length).toInt() }",
                                              // "${(widget.qty - widget.qty).toInt()}",
                                              style: TextStyle(
                                                  fontWeight: FontWeight.bold,
                                                  fontSize: 17),
                                            ),
  ...
  Future _savePickUpOrderTask() async {
    ...
    final responseBody = {
      "department_transfer_detail": widget.orderId,
      "department_transfer_packing_types": scannedPack
    };

    log("Scanned Serial code final" + json.encode(responseBody));
    try {
      var response = await http.post(
        Uri.parse("https://$finalUrl${StringConst.savePickup}"),
        headers: {
          'Content-type': 'application/json',
          'Accept': 'application/json',
          'Authorization': 'Bearer ${prefs.get("access_token")}'
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        Navigator.pop(context);
        goToPage(context, DepartmentTransferPickOrder());
  ...
  popAndLoadPage(pkOrderID) {
    Navigator.pop(context);
    Navigator.pop(context);
    goToPage(context, DepartmentPickUpOrderDetails(pkOrderID));
  }
}
