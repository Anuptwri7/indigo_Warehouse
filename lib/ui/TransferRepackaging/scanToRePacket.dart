...
import 'package:indigo_paints/ui/TransferRepackaging/removeRePackageCodes.dart';
...
class TransferScanToRePacketUI extends StatefulWidget {
  ...
  TransferScanToRePacketUI(this.masterId,this.id,this.rePackageId,this.packCodeList,this.remPackCodes,this.serialvalueDict,this.newSerialvalueDict);

  @override
  State<TransferScanToRePacketUI> createState() => _TransferScanToRePacketUIState();
}

class _TransferScanToRePacketUIState extends State<TransferScanToRePacketUI> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Transfer Rem Re-Package',style: TextStyle(fontSize: 16),),
        backgroundColor: Color(0xff2c51a4),
        actions: [
          InkWell(
            onTap: (){
              Navigator.push(context, MaterialPageRoute(builder: (context)=>TransferRemoveRepackagedPack(widget.masterId,widget.id,widget.rePackageId,widget.packCodeList,widget.remPackCodes,widget.serialvalueDict,widget.newSerialvalueDict)));
            },
            child: Center(
              child: Container(
  ...
  Future dropCurrentItem(locationCode, packCode)  async {
    ...
    final response = await http.post(
        Uri.parse('${StringConst.protocol}$finalUrl${StringConst.addRepackage}'),
        // Uri.parse('${StringConst.protocol}$finalUrl:8081${StringConst.addRepackage}'),
        headers: {
          'Content-type': 'application/json',
          'Accept': 'application/json',
          'Authorization': 'Bearer ${prefs.get("access_token")}'
...
log(response.body);
    if (response.statusCode == 401) {replacePage(LoginScreen(), context);}
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        Navigator.pop(context);
        // Navigator.pop(context);
        displayToastSuccess(msg: 'Item Repackaged Successfully');
      } else {
        // displayToast(msg: StringConst.somethingWrongMsg);
      }
    }
