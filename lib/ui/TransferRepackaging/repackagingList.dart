...
import 'package:indigo_paints/ui/TransferRepackaging/rePackAll.dart';
import 'package:indigo_paints/ui/TransferRepackaging/scanToRePacket.dart';
...
class _TransferRepackgingListUiState extends State<TransferRepackgingListUi> {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          // InkWell(
          //   onTap: (){
          //     Navigator.pushReplacement(context, MaterialPageRoute(builder: (context)=>TransferRePackAll(widget.id,packCodesGot,serialvalueDict)));
          //   },
          //   child: Center(
          //     child: Container(
          //       padding: kMarginPaddMedium,
  ...
  Future generateRePackageList() async {
    ...
    final response = await http.post(
        ...
        body: (jsonEncode({
          "re_package_qty": 1,
          "re_package_choice": "TRANSFER",
          "department_transfer_master": widget.id,
        })));
  ...
  Future<List<Results>?> pickUpOrders() async {
    ...
    log('http://${finalUrl}:8081${StringConst.transferRepackageListGetInfo}?id=&department_transfer_master=${widget.id}');
  ...
  Future<List<Result>?> rePackageList() async {
    ...
    log('http://${finalUrl}:8081${StringConst.transferRepackageListings}?id=&department_transfer_master=${widget.id}');
    // http.Response response = await NetworkHelper(
    //         '$finalUrl${StringConst.urlCustomerOrderApp}order-master?ordering=-id&limit=0&offset=0&search=$search')
    //     .getOrdersWithToken();
    print("Response Code Drop: ${response.statusCode}");
