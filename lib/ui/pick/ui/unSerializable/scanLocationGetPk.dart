...
import 'package:indigo_paints/ui/pick/ui/unSerializable/pickUpScreen.dart';
...
import '../../model/packTypoModelUnserializable.dart';
import '../../model/pack_type_model.dart';
import '../ByBatch/controller/scan_serial_controller.dart';

class ScanLocationGetPkForUnSerializable extends StatefulWidget {
  int orderId;
  int purchaseDetail;
  double qty;

  ScanLocationGetPkForUnSerializable(this.purchaseDetail, this.orderId, this.qty);

  @override
  State<ScanLocationGetPkForUnSerializable> createState() =>
      _ScanLocationGetPkForUnSerializableState();
}

class _ScanLocationGetPkForUnSerializableState
    extends State<ScanLocationGetPkForUnSerializable> {
  ...
  dropItemByBtachDetails(data) {
    return data != null
        ...
        itemBuilder: (context, index) {
          availablePackCodes.add(data[index].code);
          log(availablePackCodes.toString());
          return GestureDetector(
            ...
            goToPage(
              context,
              TestPickupByBatchUnserializable(
                widget.purchaseDetail,
                widget.qty,
                data[index].code,
                data[index].locationCode,
...
log(response.body);
    if (response.statusCode == 401) {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        return  PackTypeListUnSerializable.fromJson(jsonDecode(response.body)).results;
      } else {
